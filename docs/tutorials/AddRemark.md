...
Create a `RemarkCommandParser` class in the `seedu.simplykitchen.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.simplykitchen.model.food`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.simplykitchen.ui.FoodCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person foodToEdit = lastShownList.get(index.getZeroBased());
        Person editedFood = new Person(foodToEdit.getName(), foodToEdit.getPhone(), foodToEdit.getEmail(),
                foodToEdit.getAddress(), remark, foodToEdit.getTags());
     ...
     * {@code foodToEdit}.
     */
    private String generateSuccessMessage(Person foodToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, foodToEdit);
    }
```
